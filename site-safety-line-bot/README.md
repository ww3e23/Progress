...
打開 https://site-safety-line-bot.ww3e23.workers.dev/admin （開發版則是 `-dev` 網址）。

1. 把工程bot 拉進群組，在群裡說一句話（群組就會出現在後台）
2. 每個群組可分別開啟：
   - **即時翻譯**：中文 ↔ 外語
   - **搜尋圖片**：群內傳 `搜圖 安全帽`
   - **搜尋資料**：群內傳 `查 鋼筋搭接`
   - **氣象播報**：群內傳 `天氣`；也可設每日自動播報時間與地點
   - **排班／夜間值班**：填名單（一行一個），到點自動通知；可選每晚全數或輪值
3. 後台也可以對「這一個群」立刻發送天氣、值班或工安提醒（熱危害／高處／降雨）
