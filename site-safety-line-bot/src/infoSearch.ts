...
import { formatWeather, isWeatherQuery, weatherPlaceFromQuery } from './weather.ts'
...
export async function searchInfo(env: Env, query: string, weatherPlace = '台北', weatherLink = ''): Promise<string> {
  const q = query.trim().slice(0, 200)
  if (!q) return infoUsage()
  if (isRebarWeightQuery(q)) return rebarWeightTable()
  if (isWeatherQuery(q)) return formatWeather(weatherPlaceFromQuery(q, weatherPlace), weatherLink)
